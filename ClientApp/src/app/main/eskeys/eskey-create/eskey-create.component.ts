...
export class EskeyCreateComponent implements OnInit {
  ...
  generateEskeyReceivableDetails() {
    for (let index = 0; index < this.eskeyReceivableForm.controls.count.value; index++) {
  ...
  addOneEskeyReceivableDetail() {
    let countNewValue = (this.eskeyReceivableForm.controls.count.value as number) + 1;
  ...
  createEskey() {
    this.eskeyService.Create(this.eskeyReceivableForm.value).subscribe(result => console.log(result));
  }
